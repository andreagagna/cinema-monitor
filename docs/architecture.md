...
| `src/advisor.py` (`SeatAdvisor`) | Public API tying dates → discovery → seat-map fetch → parsing → seat selection. Returns serialisable `SeatRecommendation` objects for downstream bots/CLI. | Higher layers only talk to this orchestrator; it hides HTML/networking concerns. |
...
* **Seat suggestions**: downstream services (CLI, Telegram bot) should depend on `SeatAdvisor` or `MonitorScheduler.run_once()`, which already delivers JSON-friendly `SeatRecommendation` objects.
